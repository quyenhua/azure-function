...
resource "azurerm_private_dns_zone" "blob" {
  name                = "azure-function.blob.core.windows.net"
  resource_group_name = module.resource_group.name
}
...
module "storage_account" {
  ...
  containers_list = [
    { name = "mystore", access_type = "private" },
    { name = "blobstore", access_type = "blob" },
    { name = "containter", access_type = "container" }
  ]
  ...
  lifecycles = [
    ...
    {
      prefix_match               = ["blobstore/blobfolders"]
      tier_to_cool_after_days    = 0
